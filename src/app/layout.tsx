...
export default async function RootLayout({children}:{children:React.ReactNode}){
  ...
  return <html lang={locale}><body className="font-sans antialiased"><TranslationProvider locale={locale} messages={messages}><a href="#main" className="sr-only focus:not-sr-only focus:fixed focus:z-[100] focus:bg-gold focus:p-3 focus:text-midnight">{t("skipToContent")}</a>{!isHoangPortfolio&&<Navbar/>}<main id="main">{children}</main>{!isHoangPortfolio&&<Footer/>}</TranslationProvider></body></html>
}
