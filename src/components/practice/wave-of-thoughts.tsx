...
export function WaveOfThoughts(){
  ...
  const spawn=useCallback((node?:GraphNode)=>{if(phaseRef.current!=="experience")return;const selected=node??selectNode();setLanterns(current=>{if(current.length>=3)return current;const id=nextId.current++;return[...current,{id,nodeId:selected.id,label:selected.label[locale],x:12+Math.random()*76,y:54+Math.random()*25,drift:-7+Math.random()*14,duration:24+Math.random()*12,releasing:false}]})},[locale,selectNode]);
  ...
  useEffect(()=>{if(phase!=="experience")return;const id=window.setInterval(()=>{if(Date.now()-started.current>=EXPERIENCE_MS){finalRef.current();return}if(lanterns.length<2)spawn()},6500);return()=>window.clearInterval(id)},[phase,lanterns.length,spawn]);
  ...
  function startExperience(){setPhase("experience");phaseRef.current="experience";started.current=Date.now();spawn();scheduleBreath()}
