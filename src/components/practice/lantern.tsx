...
export type LanternState={id:number;nodeId:string;label:string;x:number;y:number;drift:number;duration:number;expiresAt:number;releasing:boolean;departing:boolean};
export function Lantern({lantern,hint,reducedMotion,onRelease}:{lantern:LanternState;hint:string;reducedMotion:boolean;onRelease:()=>void}){
  ...
  return <button className={`${styles.lantern} ${holding?styles.holding:""} ${lantern.releasing?styles.releasing:""} ${lantern.departing?styles.departing:""}`} style={{left:`${lantern.x}%`,top:`${lantern.y}%`,"--drift":`${lantern.drift}vw`,"--duration":`${lantern.duration}s`} as React.CSSProperties} onPointerDown={begin} onPointerUp={cancel} onPointerCancel={cancel} onPointerLeave={cancel} onKeyDown={event=>{if((event.key===" "||event.key==="Enter")&&!event.repeat){event.preventDefault();begin()}}} onKeyUp={event=>{if(event.key===" "||event.key==="Enter")cancel()}} aria-label={`${lantern.label}. ${hint}`} aria-describedby={`lantern-help-${lantern.id}`}><span className={styles.lanternLight}/><b>{lantern.label}</b><i/><small id={`lantern-help-${lantern.id}`}>{hint}</small>{announced&&<span className={styles.srOnly} aria-live="polite">{lantern.label}</span>}</button>
}
