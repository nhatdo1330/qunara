...
export function ResourceCard({ resource }: { resource:Resource }){
  ...
  return <article className="group overflow-hidden rounded-lg border border-white/10 bg-panel/80 transition duration-300 hover:-translate-y-1 hover:border-gold/35 hover:shadow-glow">
    <div className="relative aspect-[16/10] overflow-hidden"><Artwork type={resource.art} className="size-full transition duration-700 group-hover:scale-[1.03]"/><button onClick={()=>setSaved(!saved)} aria-label={saved?"Remove saved resource":"Save resource"} className={`absolute right-3 top-3 icon-button backdrop-blur-md ${saved?"border-gold bg-gold text-midnight":"bg-midnight/70"}`}><Bookmark size={15} fill={saved?"currentColor":"none"}/></button></div>
