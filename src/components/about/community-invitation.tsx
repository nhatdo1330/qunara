import { ArrowRight, BookOpen, Compass, Download, ExternalLink, Users } from "lucide-react";
  ...
  return <section className="about-community-invitation" id="community-invitation">
    <FinalInvitationStudy/><div className="q-shell"><p className="q-kicker">{t("kicker")}</p><h2>{t("titleOne")}<br/><em>{t("titleTwo")}</em></h2><p>{t("body")}</p>
      ...
      <div className="about-founder-links">
        <a className="about-engineering-link" href="/hoang">{t("engineering")}<ExternalLink/></a>
        <a className="about-engineering-link" href="/hoang/Hoang-Do-Resume.pdf" download>{t("resume")}<Download/></a>
      </div>
    </div>
  </section>;
}
